...
A drop-in shell MCP server that auto-yields long-running commands into managed
...
- **Long-Running Commands** (e.g., `npm run dev`, `docker build`, `sleep 60`):
  Automatically yield control back to the agent with an `execution_id` and a
  snapshot of initial output, letting the agent decide when to `read`, `wait`,
...
These client command and environment snippets are unchanged in 1.0; the
breaking changes affect tool calls and response fields, not server launch
configuration.
...
* **Output Statuses**:
  * Every status returns the cursor fields described under [Byte Cursors & Incremental Reads](#byte-cursors--incremental-reads): `start_seq`, `next_seq`, `latest_seq`, `capped`, and `evicted`. The inline snapshot advances the execution's server-side read cursor, so a following `read` or `wait` continues after it rather than repeating it.
  ...
  * If initial `stdin` delivery fails, responses for the retained execution include
...
YieldShell gives graceful termination 10 seconds before force-killing, waits up
...
Version 1.0.0 is a clean breaking release. There are no compatibility aliases
or dual-emitted legacy fields.
