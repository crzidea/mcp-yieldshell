...
* **`RingBuffer`** (`src/mcp_yieldshell/process/ring_buffer.py`):
  ...
  * Byte positions start at 1, are unique for a process record's lifetime, and are never reset — not by eviction, which only advances the oldest retained position, and not by `clear()`. A cursor aimed at evicted data resolves to the earliest retained position with `evicted` set rather than erroring.
  * `ProcessManager._read_for_response` resumes cursorless reads from `ManagedProcess.read_cursor` and then advances it, so callers can poll without tracking `next_seq`. Requests carrying an explicit `since_seq`, a `tail_lines` tail, or a narrowed `streams` selection are out-of-band: they neither read nor write that cursor. Narrowed selections must stay out-of-band because `next_seq` would otherwise advance the shared cursor past bytes on the excluded stream.
